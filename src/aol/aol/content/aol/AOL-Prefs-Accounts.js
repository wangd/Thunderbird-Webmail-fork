var gPrefAccounts =
{
    ...
    init : function()
    {
        try
        {
            ...
            this.m_strBundle = document.getElementById("stringAOLFolders");
            ...
            this.m_DebugLog.Write("AOL-Pref-Accounts : Init - this.m_bDefaultUnread "+this.m_bDefaultUnread );
                       
            //download junkmail          
            oPref.Value = null;
            WebMailPrefAccess.Get("bool","aol.bUseJunkMail",oPref);
            this.m_bDefaultSpam = oPref.Value;
            this.m_DebugLog.Write("AOL-Pref-Accounts : Init - this.m_bDefaultSpam "+this.m_bDefaultSpam );
    ...
    save : function ()
    {
        try
        {
            this.m_DebugLog.Write("AOL-Pref-Accounts : save - START");
            ...
            if (this.m_aszUserList.length>0)
            {
                ...
                {
                    WebMailPrefAccess.Set("char","aol.Account."+i+".user",this.m_aszUserList[i].szUser);
                    this.m_DebugLog.Write("AOL-Pref-Accounts.js - save - user " + this.m_aszUserList[i].szUser);
                    ...
                    WebMailPrefAccess.Set("char","aol.Account."+i+".szFolders",szFolders);
                    this.m_DebugLog.Write("AOL-Pref-Accounts.js - save - szFolders " + szFolders);
   
                }
                WebMailPrefAccess.Set("int","aol.Account.Num",this.m_aszUserList.length);
            }
            
            this.m_DebugLog.Write("AOL-Pref-Accounts : save - END");
        }
        catch(e)
        {
            this.m_DebugLog.DebugDump("AOL-Pref-Accounts : Exception in save : " 
                                          + e.name + 
                                          ".\nError message: " 
                                          + e.message + "\n"
                                          + e.lineNumber);
    ...
    getAccountPrefs : function ()
    {
        try
        {
            this.m_DebugLog.Write("AOL-Pref-Accounts : getAccountPrefs - START");
            ...
            if (WebMailPrefAccess.Get("int","aol.Account.Num",oPref))
            {
                ...
                this.m_DebugLog.Write("AOL-Pref-Accounts.js - getAccountPrefs num " + iAccountNum);
                ...
                if(iAccountNum>0)
                {
                    ...
                    for (i=0; i<iAccountNum; i++)
                    {
                        ...
                        this.m_DebugLog.Write("AOL-Pref-Accounts.js - getAccountPrefs - i " + i);
                        ...
                        if (oPref.Value)
                        {
                            ...
                            this.m_DebugLog.Write("AOL-Pref-Accounts.js - getAccountPrefs - oData.szFolders " + oData.aszFolder);   
                        }
            ...
            this.m_DebugLog.Write("AOL-Pref-Accounts : getAccountPrefs - END");
            return true;
        }
        catch(e)
        {
             this.m_DebugLog.DebugDump("AOL-Pref-Accounts : Exception in getAccountPrefs : " 
                                          + e.name + 
                                          ".\nError message: " 
                                          + e.message + "\n"
                                          + e.lineNumber);
    ...
    getAccountList : function ()
    {
        try
        {
            this.m_DebugLog.Write("AOL-Pref-Accounts : getAccountList - START");
                                    ...
                                    if (this.m_aszPrefsList)//check for prefs
                                    {
                                        ...
                                        for (j=0; j<this.m_aszPrefsList.length; j++)
                                        {
                                            if (this.m_aszPrefsList[j].szUser.search(reg)!=-1)
                                            {
                                                ...
                                                this.m_DebugLog.Write("AOL-Pref-Accounts : getUserNameList - bJunkFolder " + data.bUseJunkMail);
                                              
                                                data.aszFolder = this.m_aszPrefsList[j].aszFolder;
                                                this.m_DebugLog.Write("AOL-Pref-Accounts : getUserNameList - aszFolder " + data.aszFolder);
                                   ...
                                   if (!bFound)//prefs not found 
                                    {
                                        ...
                                        this.m_DebugLog.Write("AOL-Pref-Accounts : getUserNameList - default spam " + data.bUseJunkMail);                                        
                                       
                                        data.bDownloadUnread = this.m_bDefaultUnread;
                                        this.m_DebugLog.Write("AOL-Pref-Accounts : getUserNameList - default bUnread " + data.bDownloadUnread);                                  
                                    }
                                                                       
                                    this.m_DebugLog.Write("AOL-Pref-Accounts : getUserNameList - userName added");    
    ...
    userAdd : function ()
    {
        try
        {
            this.m_DebugLog.Write("AOL-Pref-Accounts : userAdd - START");
            ...
            this.m_DebugLog.Write("AOL-Pref-Accounts : userAdd - END");
        }
        catch(err)
        {
            this.m_DebugLog.DebugDump("AOL-Pref-Accounts : Exception in userAdd : " 
                                       + err.name + 
                                       ".\nError message: " 
                                       + err.message + "\n"
                                       + err.lineNumber);
    ...
    userClick : function ()
    {
        try
        {
            ...
            if (this.m_aszUserList.length>0)
            {
                ...
                this.m_DebugLog.Write("AOL-Pref-Accounts : userClick -  data.szUser "+ data.szUser);
                ...
                {
                    this.m_DebugLog.Write("AOL-Pref-Accounts : userClick - removing " + i);
                    var item = listFolders.getItemAtIndex(0);
                    listFolders.removeChild(item);
                }
                
                //add folder details
                this.m_DebugLog.Write("AOL-Pref-Accounts : userClick -  data.aszFolder " + data.aszFolder);
                if (data.aszFolder) 
                {       
                    for (var i=0; i<data.aszFolder.length; i++)
                    {
                        this.m_DebugLog.Write("AOL-Pref-Accounts : userListDBClick - data.aszFolder " + data.aszFolder[i] + " i "+i);
...
/**********************************************************************/    
    ...
    folderListAdd : function ()
    {
        try
        {
            ...
            if (oResult.value!=-1)
            {
                ...
                this.m_DebugLog.Write("AOL-Pref-Accounts : folderListAdd aszFolder" + this.m_aszUserList[this.m_iIndex].aszFolder);
            ...
            this.m_DebugLog.Write("YAOL-Pref-Accounts : folderListAdd - END");
        }
        catch(e)
        {
    ...
    {        
        try
        {
            this.m_DebugLog.Write("YAOL-Pref-Accounts : doRemove - START");
            
            //get selected item
            var listView = document.getElementById("listFolders");   //click item
            var iIndex = listView.selectedIndex;
            this.m_DebugLog.Write("YAOL-Pref-Accounts : doRemove - iIndex "+iIndex);
            
            //remove from array
            for (i=0; this.m_aszUserList[this.m_iIndex].aszFolder.length>i; i++)
            {
                var temp = this.m_aszUserList[this.m_iIndex].aszFolder.shift();
                this.m_DebugLog.Write("YAOL-Pref-Accounts : doRemove - temp "+temp);
                
                if (i!= iIndex)
                {
                    this.m_aszUserList[this.m_iIndex].aszFolder.push(temp);
                    this.m_DebugLog.Write("YAOL-Pref-Accounts : doRemove - pushed back ");
                }       
            }
            ...
            this.m_DebugLog.Write("YAOL-Pref-Accounts : folderListRemove - END");
            return true;
        }
        catch(e)
        {
    ...
    {
        this.m_DebugLog.Write("YAOL-Pref-Accounts : chkDownloadUreadOnChange - START");
