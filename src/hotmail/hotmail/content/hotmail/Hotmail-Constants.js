...
const patternHotmailInboxFolderID = /<td class="[\S]?ManageFoldersFolderNameCol"><a href="(.*?\?FolderID=.*?1&.*?)".*?>/i;
const patternHotmailJunkFolderID = /<td class="[\S]?ManageFoldersFolderNameCol"><a href="(.*?\?FolderID=.*?5&.*?)".*?>/i;
...
const patternHotmailInboxContent = /<div.*?ItemListContent.*?>/ig;
...
const patternHotmailMailBoxTable = /<table.*?class="[\S]?ItemListContentTable.*?".*?>[\s\S]*?<tbody([\s\S]*?)<\/tbody>[\s\S]*?<\/table>/im;
