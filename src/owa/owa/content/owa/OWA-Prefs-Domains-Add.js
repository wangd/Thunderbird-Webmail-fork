...
var gOWAAdd = 
{
    ...
    init: function ()
    {
        try
        {
            this.m_Log = new DebugLog("webmail.logging.comms", 
                                      ...
                                      "OWA Prefs");
            this.m_Log.Write("OWA-Prefs-Domains-Add : init - START");
            ...
            this.m_strBundle = document.getElementById("stringsOWAAdd");
            ...
            this.m_Log.Write("OWA-Prefs-Domains-Add : init - END");   
            return true;
        }
        catch(err)
        {
             DebugDump("OWA-Prefs-Domains-Add : Exception in init : "+ err.name 
                                                           + ".\nError message: " 
                                                           + err.message + "\n"
                                                           + err.lineNumber);
        }
    ...
    doOk: function ()
    {
        try
        {
            this.m_Log.Write("OWA-Prefs-Domains-Add : doOk - START");   

            //get uri value
            var szUri = document.getElementById("txtURL").value;               
            this.m_Log.Write("OWA-Prefs-Domains-Add : doOk - uri " + szUri);   
             
            //remove spaces
            szUri = szUri.replace(/\s/,"");
            this.m_Log.Write("OWA-Prefs-Domains-Add : doOk - space removed szUri "+ szUri); 
            
            //verify URI
            if (szUri.search(URLPatternVerify)==-1)
            {
                //error uri didn't verify
                this.m_Log.Write("OWA-Prefs-Domains-Add : doOk - uri veify failed");
            ...
            this.m_Log.Write("OWA-Prefs-Domains-Add : doOk - szDomain " + szDomain); 
     
            //remove @
            if(szDomain.search(/@/)!=-1)
            {
                szDomain = szDomain.split(/@/)[1];
                this.m_Log.Write("OWA-Prefs-Domains-Add : doOk - removing @ " + szDomain);
            }
                
            //remove spaces
            szDomain = szDomain.replace(/\s/,"");
            this.m_Log.Write("OWA-Prefs-Domains-Add : doOk - space removed szDomain "+ szDomain);
            ...
            if (szDomain.search(DomainPatternVerify)==-1)
            {
                ...
                this.m_Log.Write("OWA-Prefs-Domains-Add : doOk - domain veify failed");
            ...
            this.m_Log.Write("OWA-Prefs-Domains-Add : doOk - END");   
            return true;
        }
        catch(err)
        {
            this.m_Log.DebugDump("OWA-Prefs-Domains-Add : Exception in doOk : " 
                                           + err.name + 
                                           ".\nError message: " 
                                           + err.message + "\n"
                                           + err.lineNumber);
    ...
    doCancel: function ()
    {
        this.m_Log.Write("OWA-Prefs-Domains-Add : doCancel - START");
        
        window.arguments[2].value = false;
        window.close();
        
        this.m_Log.Write("OWA-Prefs-Domains-Add : doCancel - END");
        return true;
    },
